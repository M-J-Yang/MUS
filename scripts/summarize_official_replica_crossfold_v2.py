#!/usr/bin/env python3
"""Aggregate frozen seven-condition results, including Fold0 sample count."""
...
from summarize_official_replica_crossfold import CORE_CONDITIONS, core_conditions, fold0_conditions
...
def main() -> None:
    ...
    delta_names = (
        "utility75_minus_full",
        "utility50_minus_full",
        "utility50_minus_magnitude50",
        "drop_worst25_minus_full",
        "drop_best25_minus_full",
    )
    delta_values = {
        name: [
            (row["conditions"]["utility75"] - row["conditions"]["full"]),
            (row["conditions"]["utility50"] - row["conditions"]["full"]),
            (row["conditions"]["utility50"] - row["conditions"]["magnitude50"]),
            (row["conditions"]["drop_worst25"] - row["conditions"]["full"]),
            (row["conditions"]["drop_best25"] - row["conditions"]["full"]),
        ][index]
    ...
    result = {
        ...
        "macro_mean": macro,
        "deltas": delta_values,
        "macro_mean_deltas": macro_deltas,
    }
