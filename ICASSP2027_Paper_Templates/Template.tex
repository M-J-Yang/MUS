...
We therefore ask, in order: can DADS preserve recognition; does it beat matched controls; which shifts are necessary; and do native-frame, scale, and calibration analyses explain its behavior?
...
\begin{table}[t]
\centering
\caption{W2V2-F1/F2 test WER (\%) at 50\% retention. Random results are mean $\pm$ SD over three masks, reused for rescaling. Bold marks the lowest mean WER in each column.}
\label{tab:sourcecontrols}
\setlength{\tabcolsep}{4pt}
\input{tables/source_matched_controls.tex}
\end{table}

At 50\%, DADS lowers WER relative to Magnitude, Gradient, and unscaled Random in both W2V2-F1 and W2V2-F2. Random+Rescale reaches 15.64/14.93\% WER, below DADS at 16.01/15.58\%, which shows that retained amplitude is a separate factor. This motivates the scale analysis below rather than attributing every gain to the ranking alone.
...
\subsection{How do scale and calibration affect selection?}
\label{sec:robustness}
The matched controls show that the support of the shift and its amplitude should be examined separately. For D2V-F0, we apply the two prespecified scales to fixed DADS and Magnitude coordinates. Rescaling the same random masks also lowers WER from $15.08\pm0.31\%$ to $11.95\pm0.33\%$ at 50\%.

At 50\%, scaling Magnitude improves 13.75\% to 12.00\%, but scaling DADS worsens 12.94\% to 13.54\%; at 25\%, the effects are 13.30\% to 12.72\% and 15.20\% to 18.81\%. At 75\%, neither benefits and unscaled DADS is best (10.96\%). Scale therefore interacts with the ranking and retention budget.

Finally, we vary the calibration size while fixing the model and head. With 128 utterances, DADS gives $13.11\pm0.13\%$ WER at 50\% and $11.11\pm0.10\%$ at 75\%, within 0.2 points of the full-set results (12.94\% and 10.96\%). Top-quarter overlap with the full ranking rises from 81.9\% at 128 utterances to 91.3\% at 512 and 94.9\% at 1,024, while WER remains similar. Thus, a small calibration sample is sufficient for useful selection.

\section{Conclusion}
\label{sec:conclusion}
DADS identifies decision-aligned native shifts: CTC sensitivity preserves recognition, reversion confirms unequal reliance, and the native-frame/scale controls show that utility depends on the coordinate system and retained amplitude.
